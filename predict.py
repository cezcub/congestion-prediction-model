...
class CongestionPrediction():
    ...
    def data_process(self,FeaturePathList):
        ...
        for feature_name in FeaturePathList:
            name = os.listdir(self.datapath)[0]
            feature = np.squeeze(np.load(os.path.join(self.datapath, name))[:,:,FeaturePathList[feature_name]])
...
def parse_args():
    ...
    parser.add_argument("--data_path", default="../data/testing_data/feature", type=str, help='The path of the data file')
    parser.add_argument("--fig_save_path", default="./save_img", type=str, help='The path you want to save figure')
...
if __name__ == "__main__":
    import time
    start = time.time()
    feature_list = {'macro_region': 0, 'RUDY': 1, 'RUDY_pin': 2}
    args = parse_args()
